...
def split_residue_key(res_key):
    """
    residue_key(residue) is assumed to return (resseq, icode)
    """
...
def extract_interface_contacts(structure, binder_chain_id, target_chain_id, cutoff=10.0):
    ...
    Returns:
        ...
        binder_contact_keys: set of contacted binder residue keys
        contact_pairs: set of (target_res_key, binder_res_key)
    """
...
def compute_contacts_for_row(row, cutoff=10.0):
    ...
    try:
        ...
        target_keys, binder_keys, contact_pairs = extract_interface_contacts(
            structure=structure,
            binder_chain_id=row["binder_chain_id"],
            target_chain_id=row["target_chain_id"],
            cutoff=cutoff,
        ...
        summary_row = {
            **base,
            "n_target_contact_residues": len(target_keys),
            "n_binder_contact_residues": len(binder_keys),
        ...
        for target_key in sorted(target_keys):
            resseq, icode = split_residue_key(target_key)
            target_rows.append(
                {
                    **base,
        ...
        return summary_row, target_rows
...
if __name__ == "__main__":
    ...
    RANK1_PATH = DATA_DIR / "rank1_structures.parquet"
