...
def get_ca_atoms_by_resid(structure, chain_id):
    ...
    for residue in chain:
        if residue.id[0] != " ":  # skip heteroatoms to only look at carbon to create the correct backbone alignment from an alpha carbon.
            continue
        resid = residue.id[1]
        if "CA" in residue:
...
def rmsd_of_binder_after_alignment(struct_ref, struct_mob, align_chain, binder_chain="A"):
    ...
    sup = Superimposer() # uses Singular Value Decomposition (SVD) to find the optimal rotation and translation to superimpose two sets of atoms.
    sup.set_atoms(ref_align, mob_align)
    sup.apply(struct_mob.get_atoms()) 
