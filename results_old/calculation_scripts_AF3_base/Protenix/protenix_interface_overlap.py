...
PROTENIX_ROOT  = "/home/postyr/Assessing-AI-Scoring-Accuracy-for-De-Novo-Proteins/results/Protenix/Protenix_run_1"    # Path to protenix directory
AF3_GLOB = "*/*_model.cif"
PROTENIX_GLOB = "**/*sample_0*.cif"
SUMMARY_CSV = "contact_summary_af3_protenix.csv"

CONTACT_CUTOFF = 5.0

OUT_PREFIX = "af3_vs_protenix"
...
def main():
    ...
    protenix_files = glob.glob(os.path.join(PROTENIX_ROOT, PROTENIX_GLOB), recursive=True)
    ...
    def stem_protenix(path):
        complex_folder = os.path.basename(os.path.dirname(os.path.dirname(os.path.dirname(path)))).strip().lower()
        complex_folder = strip_egfr_suffix(complex_folder)
        return complex_folder

    af3_map = {stem_af3(f): f for f in af3_files}
    prot_map = {stem_protenix(f): f for f in protenix_files}

    # Write summary CSV
    with open(SUMMARY_CSV, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["complex_id", "af3_contacts", "protenix_contacts", "overlap_contacts"])

        for af3 in af3_files:
            sid = stem_af3(af3)
            prot_file = prot_map.get(sid)
            if not prot_file:
                print(f"[WARN] no Protenix file for {sid}")
                continue
